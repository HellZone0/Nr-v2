...
-- Tambahkan kode ini ke dalam skrip utama Anda
...
-- Simpan data Secret Fish yang sudah diperoleh
...
SecretUI:Hide() -- Sembunyikan secara default
...
local function showSecretObtained(fishName)
    local SecretWindow = WindUI:CreateWindow({
        ...
        Size = UDim2.new(0, 500, 0, 100), -- Sesuaikan ukuran
        Transparent = true,
        Draggable = false,
    })
    ...
    -- Hapus UI notifikasi setelah beberapa detik
    task.wait(5)
...
-- DETEKSI IKAN RARE
-- =========================
-- Logika ini perlu dimodifikasi agar bisa mendeteksi "Secret"
...
if REObtainedNewFishNotification then
    REObtainedNewFishNotification.OnClientEvent:Connect(function(itemId, _, data)
        if data and data.InventoryItem and data.InventoryItem.Metadata then
            ...
            if fishData and fishData.Data and fishData.Data.Tier == 7 then -- Tier 7 biasanya adalah Secret
                local fishName = fishData.Data.Name
                if not obtainedSecretFishes[fishName] then
                    obtainedSecretFishes[fishName] = os.time()
                    createSecretListUI() -- Perbarui UI
                end
